...
type BhpController struct {
	UseCase   *usecase.BhpUseCase
	Validator *config.Validator
}

func NewBhpController(useCase *usecase.BhpUseCase, validator *config.Validator) *BhpController {
	return &BhpController{
		UseCase:   useCase,
		Validator: validator,
	}
}

func (c *BhpController) Create(ctx *fiber.Ctx) error {
...
func (c *BhpController) Get(ctx *fiber.Ctx) error {
...
func (c *BhpController) GetById(ctx *fiber.Ctx) error {
	id := ctx.Params("id")
...
func (c *BhpController) Delete(ctx *fiber.Ctx) error {
	id := ctx.Params("id")
	updater := ctx.Locals("user").(string)
